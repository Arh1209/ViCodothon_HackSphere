...
def register_user(full_name: str, email: str, password: str, job_role: str, years_experience: float, education: str) -> Dict[str, Any]:
    init_db()
    ...
    cursor.execute("""
    INSERT INTO users (candidate_id, full_name, email, password_hash, job_role, years_experience, education, role, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, 'candidate', ?)
    """, (cand_id, full_name.strip(), email_clean, pwd_hash, job_role.strip(), years_experience, education.strip(), now))
